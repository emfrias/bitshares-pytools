...
if __name__ == "__main__":
 ...
 for top,delegate in enumerate(delegates["result"],1) :
  ...
  for feed in feeds :
   ...
   feedprice[ feed["asset_symbol"] ].append(float(feed[ "price" ]))
 ...
 for p in delegatefeeds : 
  assetstr = ""
  assetstr = "\n".join([ "%8s, %11.8f (med%+8.3f%%)" % (a["asset_symbol"],float(a["price"]), 100*(float(a["price"])-medianPrice[ a[ "asset_symbol" ]])/medianPrice[ a[ "asset_symbol" ]]) for a in p[ "feeds" ] ])
